...
"""根据PID协议 发送服务位为07的请求数据帧，记录汽车对该检验帧的反馈并记录到
    DTCs_pending.csv文件中，再根据协议对响应进行分析，直接得到故障码"""
...
class DTC(MyDevice):
    ...
    def get_DTC_frame(self):
        ...
        with open("DTC/DTCs_pending.csv", 'w', newline="") as csv_file:
            file_writer = csv.writer(csv_file)
            file_writer.writerow(['Original ID', temp_ID_list])

            tx = Frame(0x7DF, [0x01, 0x07])          # 07服务获取DTC故障码
    ...
    def get_real_DTC(self):                     # 获取真实DTC
        with open('DTC/DTCs_pending.csv', 'a', newline='') as csv_file:
            file_writer = csv.writer(csv_file)
